...
Context API solves prop-drilling from all of your application.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

The store is the way that Redux holds state in one place. Reducers describe how you change state in Redux. Actions are how you actually change the state.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

Thunk allows us perform asynchronous actions like API calls without having to have mutable state. The action creators will return a function instead of an action object.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

My favorite system is Redux because everything has its own place for everything.
